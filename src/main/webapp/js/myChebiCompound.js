...
//		this._container.html('').addClass("ChEBICompound");
		...
		this._table = jQuery('<table width="100%"><tbody>  </tbody></table>').appendTo(this._container);
		this._imageContainer = jQuery('<tr class="centerStructureResult">  </tr>').appendTo(this._table);
		this._imageContainer = jQuery('<td>  </td>').appendTo(this._imageContainer);
...
//	           }
	...
	//	container.children().remove();
		...
		if ( Biojs.Utils.isEmpty(data) ) {
			...
			jQuery('<tr> <td><a href="http://www.ebi.ac.uk/chebi/searchId.do?chebiId=' + data["chebiId"].value + '">' + 
					data["chebiId"].value + '</a>&nbsp;' + data["chebiAsciiName"].value + '</td> </tr>').appendTo( container );
			var starsString = "";
